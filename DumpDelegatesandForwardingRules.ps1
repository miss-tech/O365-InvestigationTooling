...
foreach ($User in $allUsers)
{
    ...
    $UserInboxRules += Get-InboxRule -Mailbox $User.UserPrincipalname | Select MailboxOwnerId, Name, Description, Enabled, Priority, HasAttachment, BodyContainsWords, SubjectContainsWords, SubjectOrBodyContainsWords, SentTo, CopyToFolder, MoveToFolder, MarkAsRead, ForwardTo, ForwardAsAttachmentTo, RedirectTo, DeleteMessage
